...
RootConfig* XmlReader::ReadXmlFile(QString& filename) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (e.nodeName() == ("trayIcon")) {
            //当有多个trayIcon情况，只取第一个trayIcon，用下面第2种方案
            //解决方案：1.创建一个TrayIconConfig对象 判断 不为空 先释放旧的 2.只取第一个 if
            if (result->trayIconConfig.iconPath == "") {
                result->trayIconConfig.iconPath = e.attribute("iconPath");
            }
...
void XmlReader::getNodes(MenuConfig* menu, QDomNode node) {
    if (node.isNull()) {
        return;
    }
    ...
    if (list.count() > 0) {
        for (int i = 0; i < list.count(); i++) {
            ...
            //得到第i个节点
            QDomNode node = list.at(i);
            //将menuConfig作为menu的子节点，增加到menu的subMenus
